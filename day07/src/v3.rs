//! V3. Similar to V2 but we don't traverse the rest of directory when
//! we know that we already have found better result.
//! In practice this turned out to not better results because
//! we only skipped such directories *two* times in the given input.
//! Actually the performance was worse (probably because of additional
//! conditional check required to determine if we skip traversla of directory)
